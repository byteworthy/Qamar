...
const INTENSITY_LABELS: Record<number, { label: string; color: string; description: string }> = {
  1: { label: "Mild", color: SiraatColors.emerald, description: "A whisper of discomfort" },
  2: { label: "Light", color: SiraatColors.emerald, description: "Noticeable but manageable" },
  3: { label: "Moderate", color: SiraatColors.sand, description: "Weighing on you" },
  4: { label: "Heavy", color: SiraatColors.clay, description: "Hard to carry" },
  5: { label: "Intense", color: SiraatColors.clayDark, description: "Overwhelming" },
};
...
export default function ThoughtCaptureScreen() {
  ...
  return (
    ...
    >
      ...
      <Animated.View entering={FadeInUp.duration(400).delay(100)} style={styles.intensitySection}>
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  ...
                  {
                    backgroundColor: isSelected ? labelInfo.color : theme.backgroundDefault,
        ...
        <ThemedText type="small" style={[styles.intensityDescription, { color: INTENSITY_LABELS[emotionalIntensity].color }]}>
          {INTENSITY_LABELS[emotionalIntensity].label}: {INTENSITY_LABELS[emotionalIntensity].description}
        </ThemedText>
      </Animated.View>

      {/* Somatic Awareness Section - appears for higher intensity */}
      {showSomaticPrompt && (
        <Animated.View entering={FadeIn.duration(300)} style={styles.somaticSection}>
          <ThemedText type="caption" style={[styles.sectionLabel, { color: theme.textSecondary }]}>
