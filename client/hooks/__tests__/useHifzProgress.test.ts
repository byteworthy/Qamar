...
describe('useHifzProgress', () => {
  ...
  describe('All juz progress (no juzNumber)', () => {
    ...
    it('should calculate overall stats correctly', () => {
      ...
      const expectedTotal = mockJuzProgress.reduce((sum, juz) => sum + juz.totalVerses, 0);
      ...
      expect(result.current.overallStats.percentageComplete).toBeCloseTo(expectedPercentage, 2);
    });
  });
