...
import { quranMetadata, userProgress, prayerPreferences } from '../shared/schema';
...
async function seedDatabase() {
  try {
    ...
    if (existingSurahs.length > 0) {
      defaultLogger.warn('[SEED] Quran metadata already exists. Skipping seeding.');
      defaultLogger.info('[SEED] To re-seed, first truncate the quran_metadata table.');
      return;
    }

    // Determine which set to use based on environment
    const surahsToSeed = process.env.SEED_MODE === 'full' ? allSurahs : testSurahs;
    ...
    defaultLogger.info(`[SEED] ✅ Successfully inserted ${surahsToSeed.length} surahs`);

    // Verify insertion
    const insertedCount = await db.select().from(quranMetadata);
    defaultLogger.info(`[SEED] Verification: ${insertedCount.length} surahs in database`);
