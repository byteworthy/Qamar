...
test.describe('Navigation', () => {
  test('should display navigation bar with logo', async ({ page }) => {
    ...
    const logoFontFamily = await logo.locator('span').evaluate(el =>
  ...
  test('should show navigation links on desktop', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: 'Reflect' }).first()).toBeVisible();
  ...
  test('should show mobile menu button on small screens', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: 'Reflect' }).last()).toBeVisible();
