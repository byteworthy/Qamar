...
test.describe('Login Flow', () => {
  ...
  test("should have submit button disabled when email is empty", async ({
    page,
  }) => {
    await page.goto("/login");
  ...
  test("should show success message after submission", async ({ page }) => {
    ...
    await expect(
      page.locator("h2", { hasText: "Check Your Email" }),
    ...
    const checkmark = page
      .locator("svg")
      .filter({ has: page.locator('path[d*="M5 13l4 4L19 7"]') });
