...
user_setup:
  - service: "Railway"
    ...
    env_vars:
      - name: RAILWAY_TOKEN
        source: "Railway dashboard → Account Settings → Tokens → Create new token"
        note: "Service token with deployment permissions"
    dashboard_config:
      - task: "Create preview service"
        location: "Railway dashboard → New Project → Empty Project → Add Service"
...
must_haves:
  truths:
    - "Pull requests automatically deploy to staging environment"
    - "Staging URL is posted as comment on PR"
    - "Staging deployment includes both frontend and backend"
  artifacts:
...
<objective>
Implement automated deployment of PR changes to staging environment for testing before merge.

Purpose: Enable testing of changes in production-like environment before merge. Stakeholders can review and test features at staging URLs without local setup. Catches environment-specific issues early.

Output: GitHub Actions workflow that deploys backend to Railway on every PR, posts staging URL in PR comments.
</objective>
