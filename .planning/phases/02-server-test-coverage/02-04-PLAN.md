...
1. **getPacingConfig** - Pacing configuration for different distress levels
   ...
   - Moderate distress → 800ms delay, 300 char max
   - Low/minimal → 500ms delay, 400 char max
   - Edge cases: undefined repetition count, unknown distress levels

2. **detectSessionFatigue** - Session exhaustion detection
   - Long duration (>45 min) → fatigue detected
   - High interaction count (>20) → fatigue detected
   - High distress + long session → fatigue detected
   - Repetition count >3 → fatigue detected
   - Crisis detected → always fatigue
   - Normal sessions → no fatigue

3. **createClosureRitual** - Gentle session endings
...
4. **shouldOfferBreak** - Break offering logic
   - Duration >30 min → offer break
   - Distress high/crisis + duration >20 min → offer break
   - Interaction count >15 → offer break
   - Repetition >2 → offer break
   - Normal conditions → no break offered
...
3. Organize tests by method (getPacingConfig, detectSessionFatigue, createClosureRitual, shouldOfferBreak)
...
2. Tests cover all 4 main methods (getPacingConfig, detectSessionFatigue, createClosureRitual, shouldOfferBreak)
