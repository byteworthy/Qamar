# Play Store Metadata (Android)

## App Name
Qamar

## Short Description (80 characters max)
Your complete Islamic companion — Quran, Arabic learning, hadith, and prayer.

## Full Description
Qamar — Your Complete Islamic Companion

Quran · Arabic Learning · Prayer · Reflection

Qamar brings together everything you need for your Islamic journey in one beautiful app. Whether you are deepening your connection with the Quran, learning Arabic from scratch, exploring hadith, or building daily spiritual habits, Qamar is designed to walk with you.

QURAN READER
• Full Quran with Arabic text and English translation
• 8 acclaimed reciters to listen and follow along
• Word-by-word audio playback for precise learning
• Tajweed color-coding to master proper recitation
• Bookmark and track your reading progress

LEARN ARABIC
• Flashcard-based vocabulary with spaced repetition (FSRS)
• Arabic Alphabet grid showing all letter forms
• Conversational Arabic tutor with 4 learning modes
• Pronunciation Coach — record yourself and get real-time feedback
• Arabic-English translator with transliteration and detailed explanations

ASK AMAR — ISLAMIC KNOWLEDGE COMPANION
• Ask questions about Islam and get thoughtful, grounded answers
• Explore topics across fiqh, seerah, and daily practice
• Responses rooted in authentic Islamic scholarship

HADITH LIBRARY
• Browse and search collections of prophetic traditions
• Save and revisit hadith that resonate with you

DAILY REFLECTIONS & JOURNALING
• Write freely in a calm, private space
• Receive thoughtful reflections on what you write
• Build a consistent journaling habit tied to your spiritual growth
• Export your reflections anytime

PRAYER & SPIRITUAL GROWTH
• Track your daily prayers and spiritual practices
• Streaks, achievements, and milestones to keep you motivated
• Celebrate consistency, not perfection

PRIVACY FIRST
• No ads, ever
• No data selling
• No tracking across apps
• Your writing and conversations stay private
• Request data deletion at any time

QAMAR PLUS ($2.99/month or $19.99/year)
• Unlimited tutor conversations and pronunciation sessions
• Full Quran audio with all 8 reciters
• All Arabic learning scenarios
• Complete Hadith library access
• Pattern insights and analytics
• Export reflections
• Lifetime option available for $49.99

Free users enjoy full access to the Quran reader, Arabic alphabet, flashcards, journaling, and prayer tracking with 3 daily tutor and pronunciation sessions included.

Built with love for the ummah. Qamar is for Muslims who want to learn, reflect, and grow — at their own pace, in their own space.

## Category & Tags
- Category: Lifestyle
- Tags: Education, Religion & Spirituality, Books & Reference, Self-Improvement

## Content Rating Notes
Recommended: Everyone. No objectionable content, no mature themes, no medical claims. Content is educational and spiritual in nature. Final rating to be set in Play Console.

## Data Safety Section

### Data Collected

**Personal Information**
- Email address (optional, for account/support purposes only)

**App Activity**
- User-generated content (journal entries, tutor conversations, Ask Amar questions)
- Audio recordings (pronunciation coach — processed for feedback)
- App interactions (feature usage for diagnostics)

**App Info and Performance**
- Crash logs and diagnostics
- Device type and OS version

### Data Not Collected
- Precise location
- Contacts or address book
- Phone number
- Social media data
- Photos or videos (beyond pronunciation audio)
- Device advertising ID
- Financial or payment info (handled entirely by Google Play)
- Browsing or search history outside the app

### Data Use Purposes
- **App Functionality**: User content (text, audio) processed to generate tutor responses, pronunciation feedback, Islamic knowledge answers, reflections, and translations
- **Analytics**: Basic device diagnostics for app stability
- **Account Management**: Optional email for support requests
- **No Advertising**: No data used for advertising
- **No Third-Party Marketing**: No data shared with third parties for marketing

### Third-Party Data Processing
- **Anthropic Claude API**: User-generated text content and audio transcriptions are sent to Anthropic's API for processing (tutor conversations, reflections, pronunciation feedback, translations, Ask Amar responses). Anthropic does not use API data for model training per their data usage policy. Data is processed per-request and not retained by Anthropic beyond the API call.

### Data Sharing
- No third-party advertising or behavioral sharing
- No sale of personal data
- Third-party processing limited to Anthropic API for core app functionality (see above)

### Security Practices
- Encrypted transmission (HTTPS/TLS)
- Encrypted storage at rest
- Users can request data deletion

### Data Retention & Deletion
- User content (entries, conversations) retained until user deletes
- Audio recordings processed for feedback and not retained long-term
- Session logs retained per retention policy (30 days)
- Subscription status managed by Google Play

### Data Deletion Requests
- Users can delete their content from within the app
- Full data deletion available on request via support@byteworthy.com
- Subscription cancellation handled in Play Store

## Subscription Note (Play Billing)
All paid tiers are offered through Google Play Billing. No external payment system is used.

### Subscription Tiers

**Free Tier**
- Full Quran reader with Arabic text and translation
- Arabic Alphabet grid with all letter forms
- Flashcard vocabulary learning
- Daily reflections and journaling
- Prayer tracking with streaks and achievements
- 3 daily sessions for tutor, pronunciation coach, and Ask Amar
- Purpose: Full core experience, generous free tier

**Qamar Plus - Monthly**
- Product ID: `noor_plus_monthly`
- Price: $2.99/month
- Unlimited Arabic tutor conversations
- Unlimited pronunciation coaching sessions
- Unlimited Ask Amar questions
- Full Quran audio with all 8 reciters
- All Arabic learning scenarios
- Complete Hadith library access
- Pattern insights and analytics
- Export reflections

**Qamar Plus - Yearly**
- Product ID: `noor_plus_yearly`
- Price: $19.99/year (save ~$16)
- Same features as monthly

**Qamar Plus - Lifetime**
- Product ID: `noor_plus_lifetime`
- Price: $49.99 (one-time purchase)
- All Qamar Plus features, permanently unlocked

---

## Legal Documents

**URLs for Play Console:**
- Terms of Service: https://byteworthy.github.io/Qamar/legal/terms.html
- Privacy Policy: https://byteworthy.github.io/Qamar/legal/privacy.html
- Support/Contact: support@byteworthy.com

**Draft legal documents available in `legal/` folder:**
- Terms of Service: `legal/TERMS_OF_SERVICE_DRAFT.md`
- Privacy Policy: `legal/PRIVACY_POLICY_DRAFT.md`
- Disclaimers: `legal/DISCLAIMERS_DRAFT.md`

---

**Last Updated**: 2026-02-16
