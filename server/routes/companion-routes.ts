...
function getValidationModeCompanionResponse(message: string): {
  ...
  return {
    response: isIslamic
      ? "[VALIDATION MODE] Assalamu alaikum! This is a placeholder response. In production, Qamar would share relevant Islamic wisdom here. Configure ANTHROPIC_API_KEY for real responses."
      : "[VALIDATION MODE] This is a placeholder companion response. Configure ANTHROPIC_API_KEY for real conversations with Qamar.",
    citations: isIslamic
      ? [
          {
            type: "quran",
...
export function registerCompanionRoutes(app: Express): void {
   ...
   *
   * Send a message to the Qamar AI companion.
  ...
  app.post("/api/companion/message", aiRateLimiter, async (req, res) => {
    try {
      ...
      if (VALIDATION_MODE && !isAnthropicConfigured()) {
        req.logger.info(
          "Validation mode: returning placeholder companion response",
        );
      ...
      await Sentry.startSpan(
        ...
        async () => {
          ...
          const isIslamic =
            detectIslamicQuery(message) || isIslamicKnowledgeQuery(message);

          if (isIslamic) {
            // Child span: Islamic knowledge search
            await Sentry.startSpan(
              {
                name: "islamic_knowledge_search",
                op: "ai.companion.knowledge",
              },
              async () => {
                ...
                if (
                  ...
                  knowledgeResult.citations.some((c) => c.type === "quran")
                ) {
                  const topVerse = knowledgeResult.citations.find(
                    (c) => c.type === "quran",
                  )!;
                  islamicContext!.relevantVerse = {
                ...
                if (
                  ...
                  knowledgeResult.citations.some((c) => c.type === "hadith")
                ) {
                  const topHadith = knowledgeResult.citations.find(
                    (c) => c.type === "hadith",
                  )!;
                  islamicContext!.relevantHadith = {
