...
import { SafetyPipeline, type SafetyPipelineInput, type SafetyPipelineOutput, type OutputValidationResult } from './safety-integration';
...
export interface OrchestrationInput {
  ...
  aiResponseGenerator: (safetyGuidance: string, pacingConfig: any) => Promise<string>;
}
...
export class CanonicalOrchestrator {
   ...
   */
  static async orchestrate(input: OrchestrationInput): Promise<OrchestrationOutput> {
    ...
    try {
      ...
      if (preProcessingResult.blocked) {
        ...
        output.response = preProcessingResult.fallbackResponse || FailureLanguage.systemPause();
        ...
        if (preProcessingResult.crisisDetected) {
          ...
          SafetyTelemetry.recordCrisisEvent(preProcessingResult.crisisDetected.level);
        }
      ...
      if (!validation.charterReport.compliant) {
        ...
        for (const violation of validation.charterReport.violations) {
          SafetyTelemetry.recordViolation(violation.category, violation.severity);
          output.telemetryEvents.push({
            type: 'violation',
        ...
        // Use fallback
        output.response = FailureLanguage.needsRefinement(input.context.distressLevel);
        output.pipelineStages.fallbackUsed = true;
        return output;
      }
      ...
      const pacingIssues = validation.issues.filter(issue => issue.startsWith('pacing:'));
      ...
      output.pipelineStages.pacingValidation = pacingIssues.length === 0 ? 'passed' : 'failed';
      log(`Pacing validation ${pacingIssues.length === 0 ? 'PASSED' : 'FAILED'}`);
      ...
      const islamicViolations = validation.charterReport.violations.filter(
        v => v.category === 'content_restriction' || v.category === 'spiritual_bypassing' || v.category === 'theological_distortion'
      );

      if (islamicViolations.length > 0) {
        log(`Islamic governance FAILED: ${islamicViolations.length} violations`);
  ...
  private static validateConversationState(
    ...
    const stateIndicators: Record<ConversationState, string[]> = {
      ...
      reflection: ['understand', 'seems like', 'what i\'m hearing', 'makes sense'],
      clarification: ['more about', 'wondering', 'curious', 'help me understand'],
...
export class OrchestrationAuditLogger {
  ...
  static getStatistics(): {
    ...
    const fallbacks = this.logs.filter(log => log.pipelineStages.fallbackUsed).length;
