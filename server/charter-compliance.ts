...
export class CharterCompliance {
  ...
  private static checkProhibitedContent(outputText: string): Violation[] {
    ...
    const allahIntentPatterns = [
      { pattern: 'allah is punishing you', rule: 'No claims about Allah\'s intent' },
      { pattern: 'allah wants you to suffer', rule: 'No claims about Allah\'s intent' },
      { pattern: 'allah doesn\'t care', rule: 'No claims about Allah\'s intent' },
      { pattern: 'this happened because allah is testing', rule: 'No claims about Allah\'s intent' },
    ];
   ...
   * Part 3: Check if AI should have slowed down
   */
  private static checkSlowDownRequirements(context: CharterContext): Violation[] {
    const violations: Violation[] = [];
    ...
    if (context.distressLevel === 'high' || context.distressLevel === 'crisis') {
      ...
      if (hasComplexExplanation && context.outputText.length > 200) {
        violations.push({
          ...
          evidence: 'Complex psychological terminology used during high distress',
          severity: 'major',
      ...
      const sentences = context.outputText.split(/[.!?]+/).filter(s => s.trim().length > 0);
   ...
   * Part 4: Check if AI should have redirected
   */
  private static checkRedirectionRequirements(context: CharterContext): Violation[] {
    const violations: Violation[] = [];

    // Check if crisis detected but CBT flow continued
    if (context.crisisDetected && context.crisisDetected.level === 'emergency') {
      const hasCBTContent = 
   ...
   * Part 8: Check Islamic content usage rules
   */
  private static checkIslamicContentUsage(context: CharterContext): Violation[] {
    const violations: Violation[] = [];
    ...
    const hadithIndicators = (context.outputText.match(/bukhari|muslim|agreed upon/gi) || []).length;
   ...
   */
  private static determineSeverity(violations: Violation[]): 'none' | 'minor' | 'major' | 'critical' {
 ...
 */
export function quickTheologicalCheck(text: string): { safe: boolean; issues: string[] } {
  ...
  const theologicalViolations = report.violations.filter(
    v => v.category === 'theological_distortion' || 
