...
function loadSeedData(): void {
  ...
  try {
    ...
    if (fs.existsSync(versesPath)) {
      ...
      const surahs: { number: number; nameEnglish: string }[] = fs.existsSync(
        surahsPath,
      )
      ...
      for (const v of verses) {
        ...
        const surahName =
          surahMap.get(v.surahNumber) || `Surah ${v.surahNumber}`;
        KNOWLEDGE_BASE.push({
          type: "quran",
...
export async function queryIslamicKnowledge(
  ...
  if (!question.trim()) {
    return {
      answer: "Please ask a question about Islamic teachings.",
      citations: [],
      confidence: 0,
    };
  }
...
async function queryInMemory(
  ...
  if (topDocs.length === 0) {
    return {
      answer:
        "I could not find specific Islamic references for that question. Try asking about patience, anxiety, gratitude, prayer, or forgiveness.",
      citations: [],
      confidence: 0,
    };
  }
...
async function queryViaQdrant(
  ...
  try {
    ...
    if (!results.length) {
      return {
        answer:
          "I could not find specific Islamic references for that question.",
        citations: [],
        confidence: 0,
      };
    }
...
function loadCachedEmbeddings(): boolean {
  try {
    ...
    for (const entry of data) {
      ...
      if (!doc) continue;
      documentIndex.set(doc.reference, {
...
function saveCachedEmbeddings(): void {
  try {
    ...
    const entries: [string, IndexedDocument][] = Array.from(
      documentIndex.entries() as any,
    );
