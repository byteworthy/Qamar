...
export function detectCrisis(text: string): CrisisDetectionResult {
  ...
  if (selfHarmDetected) {
    return {
      ...
      level: 'emergency',
      indicators,
      recommendedAction: 'Provide crisis resources immediately. Do not proceed with CBT flow. Log for review.',
  ...
  if (crisisDetected) {
    return {
      ...
      level: 'urgent',
      indicators,
      recommendedAction: 'Provide support resources. Emphasize immediate help availability. Consider pausing CBT flow.',
      requiresHumanReview: true,
    };
  }
  ...
  if (concernDetected) {
    return {
      ...
      level: 'concern',
      indicators,
      recommendedAction: 'Proceed with extra compassion. Emphasize mercy. Monitor for escalation.',
...
export const CRISIS_RESOURCES = {
  emergency: {
    ...
    message: 'You\'re in a lot of pain right now, and that pain deserves immediate support. Please reach out to someone who can help.',
    resources: [
      {
    ...
    islamicContext: 'Allah\'s mercy is greater than any darkness you\'re feeling. This moment is not the end of your story.',
  },
  urgent: {
    title: 'You Don\'t Have to Carry This Alone',
    message: 'What you\'re feeling is overwhelming, and it\'s okay to ask for help. Please consider reaching out.',
    resources: [
      {
        name: '988 Suicide & Crisis Lifeline',
        contact: 'Call or text 988',
    ...
    islamicContext: 'Prophet Yunus (AS) called out from the depths of darkness, and Allah heard him. You are not alone in your struggle.',
  },
};
...
export const SCRUPULOSITY_RESPONSE = {
  message: 'What you\'re describing sounds like waswasa - whispers that create endless doubt and anxiety about religious practice.',
  guidance: 'These obsessive doubts are not a sign of weak faith. They\'re a recognized spiritual trial. The cure is not more perfection - it\'s breaking the cycle of doubt.',
  recommendation: 'Consider speaking with a knowledgeable scholar who understands religious OCD. This is a recognized condition with clear guidance.',
  islamicPrinciple: 'Allah does not burden a soul beyond its capacity (2:286). He wants ease for you, not endless doubt.',
};
...
export function validateTheologicalSafety(text: string): { safe: boolean; violations: string[] } {
  const lowerText = text.toLowerCase();
...
export function validateAIOutput(
  output: string,
  context: { type: 'analysis' | 'reframe' | 'practice'; emotionalState?: EmotionalState }
  ...
  if (crisis.detected && crisis.level !== 'none') {
    issues.push(`Crisis language detected in output: ${crisis.indicators.join(', ')}`);
    severity = 'critical';
  }
...
export interface SafeLogEntry {
  ...
  eventType: 'thought_captured' | 'analysis_generated' | 'reframe_generated' | 'crisis_detected';
  metadata: {
...
export function getAdaptiveSafetyThresholds(
  userHistory: { crisisEventsCount: number; accountAge: number }
): SafetyThresholds {
